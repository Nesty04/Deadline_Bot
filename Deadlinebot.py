...
class DeadlineBot:
    ...
    def delete_deadline(self, message):
        ...
        self.bot.register_next_step_handler(msg, self.process_name_delete_deadline)
        

    def process_name_delete_deadline(self, message):
        args = message.text
        self.deadlines = [deadline for deadline in self.deadlines if deadline.name != args]
    ...
    def edit_notification(self, message):
        ...
        self.bot.register_next_step_handler(msg, self.process_time_edit_notification)

    def process_time_edit_notification(self, message):
        try:
            args = message.text.split()
            ...
            self.bot.send_message(message.chat.id, "Использование: <old_HH:MM> <new_HH:MM>")
